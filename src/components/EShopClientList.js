...
import { fetchAll, fetchFilters, remove } from '../actions/eshop-client.action'
import {
  getEShopClientItems,
  getEShopClientLoading,
  getEShopClientPagination,
  getEShopClientModuleFilters
} from '../reducers'
...
class EShopClientList extends Component {
  state = {
    rowKey: '_id',
    selectedRowKeys: [],
    filter: {
    ...
    return (
      <div>
        ...
        >
          ...
          <Column title="桌号" dataIndex="tableName" key="tableName" />
          <Column title="租户" dataIndex="tenantId" key="tenantId" />
          <Column title="代售商" dataIndex="consigneeId" key="consigneeId" />
          <Column title="电话号码" dataIndex="phoneNumber" key="phoneNumber" />
          <Column
            title="创建时间"
            dataIndex="createdAt"
            key="createdAt"
...
const mapStateToProps = state => ({
  items: getEShopClientItems(state),
  loading: getEShopClientLoading(state),
  pagination: getEShopClientPagination(state),
  moduleFilters: getEShopClientModuleFilters(state)
})

export default connect(mapStateToProps)(EShopClientList)
