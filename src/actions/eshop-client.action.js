...
export const LOAD_MODULE_FILTERS_SUCCESS = '[EShopClient] LoadModuleFiltersSuccess'
export const LOAD_MODULE_FILTERS_FAILED = '[EShopClient] LoadModuleFiltersFailed'
...
export const fetchAll = (condition, option, sortOpt) => dispatch => {
  ...
  return EShopClientService.fetchAll(condition, option, sortOpt)
  .then(data => {
    dispatch(loadGridDataSuccess(data))
    notification.success({
      message: '获取成功',
      description: `恭喜, 获取 EShopClient 第 ${option.skip + 1} 页数据成功!`
    })
    return data
  })
  .catch(err => {
    ...
    notification.error({
      message: '获取失败',
      description: `啊哦, 获取 EShopClient 第 ${option.skip + 1} 页数据失败!`
    })
